...
def exam_delete_view(request, exam_id):
    ...
    grades = Grade.objects.filter(exam=exam_id)
    if grades.count() > 0:
...
def exam_create_view(request):
    ...
    Output: Exam json
    """
    exam_file = str(request.data.get('exam_file'))
    file_name = "exam_files/" + exam_file
    files = Exam.objects.filter(exam_file=file_name)
    if files.count() > 0:
        data = {}
        data["failure"] = "Exam file already exists"
        return Response(data=data)
    exam = Exam()
